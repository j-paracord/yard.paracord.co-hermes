...
> **Build status:** `yard stream publish` and `yard stream read-one` are the
> intended primitives but aren't implemented yet (only `yard git` exists
> today). Until they land, use the **redis-cli fallback** at the bottom of
> this skill. The rest of this section describes the target API so the
> rewrite is trivial once the subcommands ship.

### Step 1 — Publish the question

Prefer the `yard stream` wrapper (will handle Upstash auth from `REDIS_URL`):
...
## redis-cli fallback (use until `yard stream` ships)

`redis-cli` with `--tls` can talk directly to Upstash using `$REDIS_URL`.
Requires the `redis-tools` apt package — not installed by default; if
`redis-cli` is missing, emit `run.failed` with `reason:"needs_input-unsupported"`
and let the operator provision it.

**Publish question:**
```bash
QID="q-$(openssl rand -hex 4)"
NOTIFY_KEY="xata:scout:notify:runbook-events"
redis-cli --tls -u "$REDIS_URL" XADD "$NOTIFY_KEY" '*' \
  event scout.needs_input \
  run_id "$RUN_ID" \
  question_id "$QID" \
  prompt "Which HSBC account?" \
  input_type choice \
...
**Wait for reply (blocking XREAD, 30-min timeout):**
```bash
INPUT_KEY="xata:scout:input:$RUN_ID"
# XREAD BLOCK <ms>, last-id "$" = new messages only
REPLY=$(redis-cli --tls -u "$REDIS_URL" XREAD BLOCK 1800000 STREAMS "$INPUT_KEY" '$')
# Parse the flat field/value list into JSON; loop if question_id doesn't match
```

Because raw `XREAD` returns a flat array, parsing and filtering by
`question_id` in shell is fiddly. If you need this today, spawn a tiny
Node one-liner inline:

```bash
REPLY_JSON=$(node -e '
  import("ioredis").then(({default:R})=>{
    const r=new R(process.env.REDIS_URL);
    r.xread("BLOCK","1800000","STREAMS",process.env.INPUT_KEY,"$")
      .then(res=>{
        const msg=res?.[0]?.[1]?.[0]?.[1]; // [field,val,...]
        const obj={}; for(let i=0;i<msg.length;i+=2) obj[msg[i]]=msg[i+1];
        console.log(JSON.stringify(obj)); r.disconnect();
      });
  });
' INPUT_KEY="xata:scout:input:$RUN_ID")

# Filter by QID
if [ "$(echo "$REPLY_JSON" | jq -r '.question_id')" != "$QID" ]; then
  # not my reply — re-block (bound your retries)
  :
fi
```

The node fallback relies on `ioredis` being in the consumer's bundled
deps, which it is — the consumer ships with ioredis. When `yard stream`
lands, replace both blocks with its two commands and delete this
section.
